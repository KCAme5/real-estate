...
function AgentMessagesContent() {
    ...
    useEffect(() => {
        ...
        if (conversations.length > 0 && targetId && !isNaN(targetId)) {
            const selected = conversations.find((c: Conversation) => Number(c.id) === targetId);
            if (selected && (!activeConversation || activeConversation.id !== selected.id)) {
                setActiveConversation(selected);
                fetchMessages(selected.id);
                setTimeout(() => inputRef.current?.focus(), 300);
            }
        }
    }, [conversations, conversationIdParam, activeConversation?.id]);
            ...
            <div className={`w-full md:w-96 border-r border-[#1E3A5F] flex flex-col bg-[#0B192F] ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
                ...
                <div className="p-4 border-b border-[#1E3A5F]">
                    <div className="flex items-center justify-between mb-4">
                        <h1 className="text-2xl font-semibold">Client Messages</h1>
                    </div>
                ...
                {activeConversation ? (
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-gray-400">Choose a client conversation from the list to start messaging</p>
                        </div>
                    </div>
                )}
            </div>
