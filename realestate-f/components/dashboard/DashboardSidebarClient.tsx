...
export default function DashboardSidebarClient() {
    ...
    return (
        <aside className="flex flex-col h-full bg-card border-r border-border shadow-2xl w-72">
            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                <nav className="space-y-1.5 pt-4">
                    <p className="px-4 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-6">Main Menu</p>
                    {navigation.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                className={`flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all duration-300 group ${active
                                    ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-500/20 font-bold'
                    ...
                    {user?.user_type === 'agent' ? (
                        <div className="p-5 bg-emerald-500/5 rounded-3xl border border-emerald-500/10 space-y-3 relative overflow-hidden group">
                            ...
                            <p className="text-[11px] text-muted-foreground leading-relaxed font-bold">
                                Keep your listings updated with quality photos to increase inquiries by up to <span className="text-emerald-600 font-black">45%</span>.
                            </p>
                        </div>
                    ) : (
                        <div className="p-5 bg-blue-500/5 rounded-3xl border border-blue-500/10 space-y-3 relative overflow-hidden group">
                            ...
                            <p className="text-[11px] text-muted-foreground leading-relaxed font-bold">
                                Save properties to get personalized alerts and priority access to <span className="text-blue-600 font-black">exclusive</span> deals.
                            </p>
                        </div>
                    )}
                </div>
            ...
            <div className="p-6 border-t border-border bg-background/50 backdrop-blur-xl">
                <div className="p-4 bg-muted/50 rounded-2xl flex items-center gap-3 border border-border/30">
                    <div className="w-10 h-10 rounded-xl bg-emerald-600/10 text-emerald-600 flex items-center justify-center font-black text-lg border border-emerald-500/20 shadow-inner overflow-hidden relative">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-black text-foreground truncate tracking-tight">{user?.first_name} {user?.last_name}</p>
                        <p className="text-[10px] text-muted-foreground font-bold truncate uppercase tracking-tighter">{user?.email}</p>
                    </div>
                </div>
            </div>
        </aside>
